...
-- This creates an improved view for SKU matching with clear original vs matched SKU fields

-- 1. Create the enhanced main SKU matching view
...
-- 2. Create a view for pending SKU matches (not processed yet)
...
-- 4. Create a view for no-match items
...
-- 5. Create a view for manual review items
CREATE OR REPLACE VIEW manual_review_sku_items AS
SELECT 
    imei,
    original_sku,
    sku_matched,
...
-- 6. Create a summary view for SKU matching statistics
CREATE OR REPLACE VIEW sku_matching_summary AS
SELECT 
