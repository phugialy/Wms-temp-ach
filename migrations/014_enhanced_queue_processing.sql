-- ========================================
-- ENHANCED QUEUE PROCESSING FUNCTION
-- ========================================

-- Enhanced function to process all pending queue items and create actual database records
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            SELECT id INTO location_id FROM "Location" WHERE name = location LIMIT 1;
            ...
            SELECT id INTO item_id FROM "Item" WHERE imei = queue_item.raw_data->>'imei';
            ...
            SELECT id INTO inventory_id FROM "Inventory" WHERE item_id = item_id AND location_id = location_id;
            ...
            ELSE
                ...
                UPDATE "Inventory" 
                SET quantity = quantity + (SELECT quantity FROM "Inventory" WHERE id = inventory_id)
        ...
        EXCEPTION
            WHEN OTHERS THEN
                ...
                -- Log the error (you might want to add error logging here)
                RAISE NOTICE 'Failed to process queue item %: %', queue_item.id, SQLERRM;
        END;
    END LOOP;
