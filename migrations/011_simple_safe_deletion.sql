-- Migration 011: Simple Safe Deletion System
-- This migration creates safe deletion functions without trying to disable system triggers
-- Focuses on the core issue: providing reliable deletion functionality

-- ========================================
-- STEP 1: CREATE SAFE DELETION FUNCTION
-- ========================================

-- Create a simple, safe deletion function that works around trigger issues
...
BEGIN
    ...
    -- Delete in safe order (respecting foreign key constraints)
    BEGIN
        -- 1. Delete DeviceTest records (child of Item)
        DELETE FROM "DeviceTest" WHERE item_id = item_id;
        GET DIAGNOSTICS temp_count = ROW_COUNT;
        deleted_count := deleted_count + temp_count;
        
        -- 2. Delete Inventory records (child of Item)
        DELETE FROM "Inventory" WHERE item_id = item_id;
        GET DIAGNOSTICS temp_count = ROW_COUNT;
        deleted_count := deleted_count + temp_count;
        
        -- 3. Delete imei_inspect_data records (related to IMEI)
        DELETE FROM imei_inspect_data WHERE imei = target_imei;
        GET DIAGNOSTICS temp_count = ROW_COUNT;
        deleted_count := deleted_count + temp_count;
        
        -- 4. Delete imei_units records (related to IMEI)
        DELETE FROM imei_units WHERE imei = target_imei;
        GET DIAGNOSTICS temp_count = ROW_COUNT;
        deleted_count := deleted_count + temp_count;
        
        -- 5. Delete imei_sku_info records (related to IMEI)
        DELETE FROM imei_sku_info WHERE imei = target_imei;
        GET DIAGNOSTICS temp_count = ROW_COUNT;
        deleted_count := deleted_count + temp_count;
        
        -- 6. Delete imei_data_queue records (related to IMEI)
...
-- ========================================
-- STEP 2: CREATE SAFE BULK DELETION
-- ========================================
...
-- ========================================
-- STEP 3: CREATE SAFE NUCLEAR DELETE
-- ========================================
...
DECLARE
    ...
    temp_count INTEGER;
BEGIN
    -- Delete all data in safe order (respecting foreign key constraints)
...
'Safe deletion of IMEI data that respects foreign key constraints. No archiving, direct deletion only.';

COMMENT ON FUNCTION safe_bulk_delete_imei_data(TEXT[]) IS 
'Safe bulk deletion of multiple IMEIs that respects foreign key constraints. No archiving, direct deletion only.';

COMMENT ON FUNCTION safe_nuclear_delete_all_data() IS 
'Safe nuclear deletion of all data that respects foreign key constraints. No archiving, direct deletion only.';
...
DO $$
BEGIN
    RAISE NOTICE 'Migration 011: Simple Safe Deletion System completed successfully';
    ...
    RAISE NOTICE 'No system triggers were modified - this is a safe approach!';
END $$;
