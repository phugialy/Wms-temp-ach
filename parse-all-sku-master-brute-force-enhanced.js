...
class BruteForceSkuMasterTagParser {
  constructor(dbClient) {
    ...
    this.stats = {
      ...
      tagToSkus: new Map() // Track which SKUs have each undefined tag
    };
  }
  ...
  async parseSingleSku(sku) {
    ...
    if (undefinedTags.length > 0) {
      ...
      undefinedTags.forEach(tag => {
        ...
        // Track which SKUs have each undefined tag
...
async function parseAllSkuMasterBruteForce() {
  ...
  try {
    ...
    console.log('5. Repeat until you reach desired coverage!');
