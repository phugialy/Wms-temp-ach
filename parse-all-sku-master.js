...
class ProductionSkuMasterTagParser {
  ...
  async parseAllSkuMaster() {
    console.log('🚀 Starting PRODUCTION parsing of ALL SKU_MASTER entries...');
    ...
    console.log(`📊 Found ${this.stats.total} SKUs to parse (PRODUCTION MODE)`);
  ...
  categorizeTag(tagValue) {
    ...
    // Carrier tags - Check FIRST before capacity (to catch 4G, 5G, WIFI)
    // Based on ACTUAL analysis of 1,375 SKUs - only what exists
    ...
    if (['FOLD3', 'FOLD4', 'ZFLIP3', 'ZFLIP4', 'S10', 'S21', 'S22', 'S23', 
         'IPHONE13', 'IPHONE14', 'IPHONE15', 'PIXEL6', 'PIXEL7', 'PIXEL8'].includes(tag)) {
      return 'MODEL';
    }
    
    // iPad models - Enhanced recognition (individual tags)
    if (tag === 'IPAD' || tag.startsWith('IPAD')) {
      return 'MODEL';
    }
    
    // Model variants that are part of model identification
    if (['PRO', 'AIR', 'MINI', 'ULTRA', 'PLUS', 'MAX'].includes(tag)) {
      return 'MODEL';
    }
    
    // Model size indicators that are part of model identification
    if (['9.7', '10.2', '10.9', '11', '12.9', '13', '14', '15'].includes(tag)) {
      return 'MODEL';
    }
    
    // Generation indicators that are part of model identification
    if (/^\d+[STNDRDTH]+$/.test(tag) || /^\d+GEN$/.test(tag)) {
      return 'MODEL';
    }
    ...
    // Color tags - Based on ACTUAL 1,375 SKU analysis - only what exists
    // 27% BLACK, 8% SILVER, 6% BLUE, 6% WHITE, 5% GREEN, 5% SPACE_GRAY
    if (['BLK', 'BLACK', 'WHT', 'WHITE', 'GLD', 'GOLD', 'SLV', 'SILVER', 
         'GRN', 'GREEN', 'BLU', 'BLUE', 'PNK', 'PINK', 'PUR', 'PURPLE', 
         'GRY', 'GRAY', 'SG', 'SPACE_GRAY', 'ROSE', 'RED', 'ORANGE', 
         'YELLOW', 'BROWN', 'TAN', 'CREAM', 'STARLIGHT'].includes(tag)) {
      return 'COLOR';
    }
    
    // Post-fix tags - Enhanced for your patterns
    if (['VG', 'LL', 'A', 'TEST', 'GRADE-A', 'GRADE-B', 'OPENBOX', 'LIKE-NEW', 'UL', 'ACCEPTABLE'].includes(tag)) {
      return 'POSTFIX';
    }
...
async function parseAllSkuMaster() {
  ...
  try {
    ...
    // Parse ALL SKU_MASTER data
    console.log('\n🚀 Starting PRODUCTION parsing of ALL SKUs...');
    const parser = new ProductionSkuMasterTagParser(client);
    const result = await parser.parseAllSkuMaster();

    console.log('\n🎯 PRODUCTION Parsing Results:', result);
    ...
    console.error('❌ Error during production parsing:', error.message);
