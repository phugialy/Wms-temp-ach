...
class OperatorService {
    ...
    async initialize() {
        try {
            ...
            console.error('❌ Failed to initialize OperatorService:', error.message);
            throw error;
        }
    }
     ...
     */
    async shippingQuickPickup(imei, operatorName = 'SHIPPING-OP') {
        try {
            ...
            console.error(`❌ Shipping pickup failed for ${imei}:`, error.message);
            throw error;
        }
    }
     ...
     */
    async postfixUpdate(imei, grade, operatorName = 'POSTFIX-OP') {
        try {
            ...
            console.error(`❌ Postfix update failed for ${imei}:`, error.message);
            throw error;
        }
    }
     ...
     */
    async repairUpdate(imei, newLocation, repairNotes, operatorName = 'REPAIR-OP') {
        try {
            ...
            console.error(`❌ Repair update failed for ${imei}:`, error.message);
            throw error;
        }
    }
     ...
     */
    async inspectorUpdate(imei, updates, operatorName = 'INSPECTOR-OP') {
        try {
            ...
            console.error(`❌ Inspector update failed for ${imei}:`, error.message);
            throw error;
        }
    }
     ...
     */
    async updateDeviceLocation(imei, newLocation, operatorName, reason) {
     ...
     */
    async logOperatorAction(imei, role, actionType, details) {
        await this.client.query(`
