...
export class IntegratedBackgroundProcessor {
  ...
  /**
   * Process device in background (non-blocking) - works with existing imei_data_queue
   */
  ...
  private async processSkuMatching(imei: string, deviceData: FlexibleDeviceInput): Promise<void> {
    try {
      ...
      // Update status to processing in existing imei_data_queue
      await this.updateDeviceStatus(imei, 'processing');
      
      // Run SKU matching
      const skuResult = await this.skuMatchingService.matchDevice(deviceData);
      
      // Store SKU matching results in existing sku_matching_results table
  ...
  /**
   * Update device processing status in existing imei_data_queue table
   */
  ...
  private async handleError(imei: string, error: any): Promise<void> {
    try {
      // Update status to failed in existing imei_data_queue
      await this.updateDeviceStatus(imei, 'failed');
      
      // Store error details in existing imei_data_queue
      const query = `
        UPDATE imei_data_queue 
  ...
  /**
   * Get device processing status from existing tables
   */
  async getDeviceStatus(imei: string): Promise<DeviceProcessingStatus | null> {
    try {
      const query = `
        SELECT 
          raw_data->>'imei' as imei,
          input_status,
          error_message,
          smr.matched_sku,
          smr.match_score,
        ...
        LEFT JOIN sku_matching_results smr ON raw_data->>'imei' = smr.imei
        WHERE raw_data->>'imei' = $1
      `;
  ...
  /**
   * Get processing statistics from existing tables
   */
  ...
  async retryFailedDevices(): Promise<BackgroundProcessingResult> {
    ...
    try {
      // Get failed devices from existing imei_data_queue
      const query = `
        SELECT raw_data
        FROM imei_data_queue
      ...
      for (const deviceRow of failedDevices) {
        try {
          const rawData = deviceRow.raw_data;
          const deviceData: FlexibleDeviceInput = {
            ...
            device_notes: rawData.device_notes,
            working_status: rawData.working_status,
            battery_health: rawData.battery_health,
            source: rawData.source || 'api'
          };
          
          await this.processSkuMatching(rawData.imei, deviceData);
          processed++;
          
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : String(error);
          errors.push(`${deviceRow.raw_data.imei}: ${errorMessage}`);
        }
      }
...
export default IntegratedBackgroundProcessor;
