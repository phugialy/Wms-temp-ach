-- Simple queue processing function without ON CONFLICT
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Insert into product table (SKU can be NULL for unknown products)
      ...
      -- Insert into device_test table (only if working status is valid)
      IF queue_item.raw_data->>'working' IS NOT NULL 
         AND queue_item.raw_data->>'working' != 'PENDING'
         AND queue_item.raw_data->>'working' != '' THEN
        
        -- Check if device_test record already exists
        IF NOT EXISTS (SELECT 1 FROM device_test WHERE imei = imei_val) THEN
      ...
      -- Insert into sku_matching_queue (check if exists first)
      IF NOT EXISTS (SELECT 1 FROM sku_matching_queue WHERE imei = imei_val) THEN
